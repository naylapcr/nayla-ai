...
export default function ProfitChart() {
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm relative h-full flex flex-col justify-between min-h-[360px]">
      ...
      {/* ✅ PROGRESS BARS — posisi benar, di luar header */}
      <div className="space-y-3 mb-4">
        <div>
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="text-xs font-black text-gray-700">12,485</span>
          </div>
          <Progress value={72} className="h-2 bg-gray-100 [&>div]:bg-[#93c5fd] [&>div]:rounded-full" />
        </div>
        <div>
          <div className="flex justify-between items-center mb-1.5">
            <div className="flex items-center gap-1.5 text-xs font-bold text-gray-500">
            ...
            <span className="text-xs font-black text-gray-700">$68,760</span>
          </div>
          <Progress value={58} className="h-2 bg-gray-100 [&>div]:bg-[#4f46e5] [&>div]:rounded-full" />
        </div>
      </div>
        ...
        <div className="absolute inset-0 flex items-end justify-between pl-10 pr-2 pb-5 z-10">
          {bars.map((data, i) => (
            <div key={i} className="flex-1 flex flex-col items-center relative h-[84%] justify-end max-w-[52px]">
              ...
              <span className="absolute -bottom-6 text-xs font-bold text-gray-500">{data.day}</span>
            </div>
          ))}
        </div>
      </div>
